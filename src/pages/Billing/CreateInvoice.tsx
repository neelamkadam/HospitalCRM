...
export const CreateInvoice: React.FC = () => {
  ...
  const { getData: GetInvioceApi, isLoading: loading } = useGetApi<any>("");
  console.log("🚀 ~ CreateInvoice ~ loading:", loading);
  ...
  const fetchInvoice = async () => {
    if (invioceId) {
      const response: any = await GetInvioceApi(
        `${API_CONSTANTS.BILLINGS.INVOICE}/${invioceId}`
      );
      if (response?.data.success) {
        ...
        setSelectedOption({
          value: response?.data.invoice.clientId._id,
          label: response?.data.invoice.clientId.name,
        });
  ...
  const handleCreateInvoice = async () => {
    try {
      ...
      const url = invioceId
        ? `${API_CONSTANTS.BILLINGS.GET_INVOICE}/${invioceId}`
        : undefined;
        ...
        >
          ...
          <div className=" mb-[1px] md:mb-1 lg:mb-4 xl:mb-4 2xl:mb-4">
            <div
              ...
              className="rounded-xl shadow-[0px_1px_2px_0px_rgba(16,24,40,0.04)] overflow-x-auto overflow-y-auto scrollbar-thin scrollbar-thumb-neutral-200 scrollbar-track-white border !border-[#E6E7E9]"
              style={{
                height: "calc(68vh - 200px)",
                // minHeight: "calc(68vh - 200px)",
                // maxHeight: "calc(68vh - 200px)"
              }}
            >
