...
const OrganizationDetails: React.FC = () => {
    ...
    useEffect(() => {
        ...
        // fetchReports({ apiType: "users", currentPageValue: true });
        // fetchReports({ apiType: "clients", currentPageValue: true });
    ...
    const fetchReports = async ({
        apiType = "users", // Default to "users"
        currentPageValue = false,
    }: {
        apiType: "users" | "clients";
        ...
        const endpoint =
            apiType === "clients"
                ? `${API_CONSTANTS.GET_CLIENTS}?${params}`
                : `${API_CONSTANTS.GET_Users}?${params}`;

        try {
            const response: any = await GetReportApi(endpoint);
            if (response?.data.success) {
                // Dynamically pick 'clients' or 'users' from response.data
                const key = apiType === "clients" ? "clients" : "users";
                const newItems = response.data[key]?.items || [];

                if (currentPage === 1) {
                    apiType === "clients" ? setClientsData(newItems) : setUsersData(newItems);
                } else {
                    apiType === "clients"
    ...
    //  const fetchReports1 = async (currentPageValue = false) => {
    //     const pageSize = 15;
    //     const page = currentPageValue ? "1" : currentPage.toString();
    //     const params = new URLSearchParams({
    //         per_page: pageSize.toString(),
    ...
    //         );
    //         if (response?.data.success) {
    //             const newClients = response.data.users.items || [];
    //             if (currentPage === 1) {
    //                 setClientsData(newClients);
    //             } else {
    //                 setUsersData((prev: any) => [...prev, ...newClients]);
    //             }
    ...
    const cloumnsClientsData = React.useMemo<ColumnDef<any>[]>(() => {
        const baseColumns: ColumnDef<any>[] = [
            ...
            },
            {
                ...
                enableSorting: false,
                meta: { className: "hidden md:table-cell" },
                accessorFn: (row) => row?.original?.email || row?.original?.userDetails?.email || "N/A",
                cell: ({ row }) => (
                    <div className="flex">
                        <span className="text-[14px] font-regular truncate text-sm text-[#8C929A] mt-1">
            ...
            },
            {
                ...
                accessorFn: (row) => row?.original?.phone || row?.original?.phoneNumber || "N/A",
                cell: ({ row }) => (
                    <div className="flex">
                        <span className="text-[14px] font-regular truncate text-sm text-[#8C929A] mt-1">
    ...
    const dataToUse = activeTab === "clients" ? clientsData : usersData;
    const dataToUseColumns = activeTab === "clients" ? cloumnsClientsData : columnsUsersData;
